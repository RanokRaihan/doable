...
interface PaymentsReceivedClientProps {
...
export function PaymentsReceivedClient({
  payments,
  meta,
  currentFilters,
}: PaymentsReceivedClientProps) {
      ...
      {payments.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center rounded-xl border border-dashed border-slate-200 bg-slate-50">
          {hasFilter ? (
            ...
            <>
              ...
              <p className="text-sm text-slate-500 mt-1 max-w-xs">
                Payments received for completed tasks will appear here.
              </p>
            </>
          )}
        </div>
        ...
        <div className="space-y-3">
          {payments.map((payment) => (
            <PaymentCard key={payment.id} variant="received" payment={payment} />
          ))}
        </div>
      )}
